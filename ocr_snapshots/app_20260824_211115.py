...
def _extract_text_from_file_bytes(file_bytes, ext):
    ...
    def _repair_incomplete_rows(table_md, max_repairs=12):
        """Re-read rows that came back with holes, one row at a time.

        A whole-page read spreads attention thinly and silently drops values,
        which then shifts a row's remaining values into the wrong columns.  Asking
        about a single named row, with its expected columns spelled out, is a much
        easier question.  A repair is accepted only when it recovers more values
        than the original row had, so this can never remove data.
        """
        ...
        for row in grid[1:]:
            ...
            if not label or filled >= len(columns) or repairs_left <= 0:
            ...
            ask = (
                ...
                + "\n\nRead across that one row and report its value for each column. Copy each value exactly "
                "as printed, including every digit after a decimal point and any flag such as H or L printed "
                "beside it. If a column genuinely has no value in this row, use an empty string.\n\n"

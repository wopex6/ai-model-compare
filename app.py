...
def _extract_text_from_file_bytes(file_bytes, ext):
    ...
    transcribe_prompt = (
        ...
        "Trailing columns such as Reference and Units are often sparse: an entry is printed beside only some "
        "rows. Keep each entry on the line it is printed on — never move a unit or reference up or down onto a "
        "neighbouring row's line, even if it seems to belong there.\n\n"
    ...
    structure_prompt = (
        ...
        "8. Reference-range and unit cells belong to the row they sit on. If a row's unit or reference cell is "
        "empty, emit an empty cell - never copy the unit or reference from the row above or below. A blank Units "
        "cell must stay blank even when the row's unit looks obvious.\n"
        "9. Ignore surrounding prose, names, addresses, phone numbers, identifiers and footnotes.\n\n"
        "Transcription:\n"
        "{transcription}\n\n"
